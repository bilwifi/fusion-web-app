...
class send_push_notifications_list_screens_class extends WP_List_Table
{
    ...
    public function __construct()
    {
        $this->set_table();
        parent::__construct([
            'singular' => 'screens', //singular name of the listed records
            'plural'   => 'Screen', //plural name of the listed records
    ...
    {
        global $wpdb;
        self::$table = $wpdb->prefix . FUSION_WA_T_SCREENS;
    }
    ...
    function column_title($item)
    {
        ...
        if ($action === __('Remove', FUSION_WA_SLUG)) {
            $actions['edit'] = '<a href="' . FUSION_WA_URL . '-screens&action=edit-app-screen&id=' . absint($item['id']) . '">' . __('Edit', FUSION_WA_SLUG) . '</a>';
        } else {
     ...
     * @return string
     */
    function column_requires_id($item)
    {
        return $item['requires_id'] == 0 ? __('No', FUSION_WA_SLUG) : __('Yes', FUSION_WA_SLUG);
    }
    ...
    function get_columns()
    {
        return [
            ...
            'description' => __('Description', FUSION_WA_SLUG),
            'screen' => __('Route', FUSION_WA_SLUG),
            'requires_id' => __('Requires id', FUSION_WA_SLUG),
            'updated_at' => __('Last edit', FUSION_WA_SLUG)
        ];
    }
    ...
    public function process_bulk_action()
    {
        ...
        if ($this->current_action() === 'bulk-delete') {
            ...
            wp_redirect($this->root_path . '-screens');
            exit;
        }
