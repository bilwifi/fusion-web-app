...
class send_push_notifications_list_recipients_class extends WP_List_Table
{

    public static $table;
    public $root_path = FUSION_WA_URL . '-recipients';
    ...
    public function __construct()
    {
        $this->set_table();
        parent::__construct([
            'singular' => 'Recipient', //singular name of the listed records
            'plural'   => 'Recipients', //plural name of the listed records
    ...
    function get_views()
    {
        ...
        $active = remove_query_arg('filtered');
        $views['all'] = "<a href='{$active}' {$class} >". __('All', FUSION_WA_SLUG) ." (" . self::record_count() . ")</a>";
    ...
    {
        global $wpdb;
        self::$table = $wpdb->prefix . FUSION_WA_T_RECIPIENTS;
    }
     ...
     * @return string
     */
    function column_name_device($item)
    {
        // create a nonce
        $actions = [];
        $title = '<strong>' . $item['name_device'] . '</strong>';
        $delete_nonce = wp_create_nonce('recipient_action_single_record');
        $action = (!empty($_REQUEST['filtered']) ? ($_REQUEST['filtered'] == 'trashed' ? __('Delete forever', FUSION_WA_SLUG) : 'Remove') : __('Remove', FUSION_WA_SLUG));
    ...
    function get_columns()
    {
        return [
            ...
            'name_device' => __('Name device', FUSION_WA_SLUG),
            'token' => __('Token', FUSION_WA_SLUG),
            'platform' => __('Platform', FUSION_WA_SLUG),
            'installation_id' => __('Installation ID', FUSION_WA_SLUG),
            'updated_at' => __('Registered on', FUSION_WA_SLUG)
        ];
    }
    ...
    public function process_action()
    {
        ...
        if ($this->current_action() === 'delete-single') {
            ...
            if (!wp_verify_nonce($nonce, 'recipient_action_single_record')) {
